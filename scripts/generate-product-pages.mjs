...
const productTemplateSource = read("product.html");
const shopTemplateSource = read("shop.html");
const photothequeTemplateSource = read("phototheque.html");
const otherPublicSources = new Map([
  ["index.html", read("index.html")],
  ["merci.html", read("merci.html")],
  ["confidentialite.html", read("confidentialite.html")],
...
function syncConfiguredFallbacks(source) {
  let html = source;
...
const template = syncConfiguredFallbacks(productTemplateSource);
const shopTemplate = syncConfiguredFallbacks(shopTemplateSource);
const photothequeTemplate = syncConfiguredFallbacks(photothequeTemplateSource);
const otherPublicPages = new Map(
  Array.from(otherPublicSources, ([name, source]) => [name, syncConfiguredFallbacks(source)]),
);
...
function productMarkup(product) {
  ...
  const description = publicProductDescription(product);
  ...
  const action = stock.available
    ? `<a class="btn btn-whatsapp btn-lg" id="pdWhatsapp" data-stateful data-stateful-delay="600" href="${escapeHtml(whatsappLink(product, defaultSize))}" target="_blank" rel="noopener"><svg class="icon"><use href="#i-chat"></use></svg>Commander sur WhatsApp</a>`
...
function createPage(product) {
  ...
  let page = template
    ...
    .replace(/<div class="product-detail" id="productDetail">[\s\S]*?<\/div>\s*<\/div>\s*<\/section>/, `${productMarkup(product)}\n    </div>\n  </section>`);
...
const generated = new Map(PRODUCTS.map((product) => [`${product.slug}.html`, createPage(product)]));
